...
個別コメントAPI (`/p/{pid}/c/{cid}/t/{sk_token}`) で1件ずつ取得。
**upvote/downvote 数と投票トークンが欠落**するため、2ページ目以降はグレーアウト表示。

### WAF 緩和と WebView プロキシの試み（2026-02-27〜28）
- 2026-02-27: Cloudflare WAF 緩和を確認。WebView テスト画面（可視）で `?nxc=` が正常動作
- 2026-02-28: 非表示 WebView プロキシ (`WebViewCommentProxy.js`) を実装・検証
  - 初期ロード（`source={{ uri }}` prop）: 成功（result ページ読み込み + コメント抽出）
  - プログラム的遷移後の JS 実行: **iOS で全滅**
    - `injectJavaScript('location.href=...')` → `onLoadEnd` が発火しない
    - `injectedJavaScript` prop（WKUserScript）→ 遷移後に実行されない
    - `source` prop の URI を state で変更 → `onLoadEnd` が発火しない
  - 広告 iframe の `about:srcdoc` / `data:` URI が `Linking.openURL` に漏れる問題も発生
- **結論: 断念。個別コメントAPI方式を維持**
...
### ~~13. 非表示 WebView プロキシで `?nxc=` ページネーション~~
**棄却理由（2026-02-28 実測）:**
- WAF 緩和後、非表示 WebView (`WebViewCommentProxy.js`) で `?nxc=` ページネーションを試行
- 初期ロード（`source={{ uri }}` prop）: 成功（20件のコメント + upvote/downvote 取得）
- 2ページ目遷移: **iOS で全方式が失敗**
  - `injectJavaScript('location.href="...?nxc=..."')` → `onLoadEnd` が発火しない → タイムアウト
  - `injectedJavaScript` prop（WKUserScript として登録）→ 遷移後に実行されない
  - `source` prop の URI を state で変更（正規のナビゲーション方法）→ `onLoadEnd` が発火しない
- 追加の問題: 広告 iframe の `about:srcdoc` / `data:` URI が `Linking.openURL` に漏れるエラー多発
  - `originWhitelist={['*']}` で抑制可能だが、ナビゲーション後の JS 実行不可は解消せず
- **結論:** iOS WKWebView の非表示（1x1px）インスタンスではプログラム的ナビゲーション後のコールバックが動作しない
...
**?nxc= ページネーションによる upvote/downvote 取得はアプリからは不可能。**

### 現行方式（確定）
- **1ページ目（20件）**: 完全表示（upvote/downvote/token あり、good/bad 投票可能）
- **2ページ目以降**: 個別コメント API で取得（本文・日時・派閥のみ、upvote/downvote なし → グレーアウト表示）
- `getMoreComments()` は**動作中**（アプリの fetch は個別コメント API にアクセス可能）

### ブロックの構造
- `?nxc=` パラメータ: **`sk_anie` クッキー**（正規の投票で発行）+ WAF パスの両方が必要（2026-03 情報）
  - 以前は `sk_vote=1` だけで取り放題だったが、現在は `sk_anie` が必須に変更された
  - React Native の fetch はブラウザのクッキージャーを持たないため `sk_anie` の自動送信が不可能
  - 投票 POST レスポンスの `Set-Cookie` から `sk_anie` を手動取得する案もあるが、`Cookie` ヘッダー付き GET は Cloudflare 空ボディ問題に当たる可能性が高い
- iOS 非表示 WebView: プログラム的遷移後に JS 実行不可（WKWebView の制約）
...
| `apps/mobile/src/components/WebViewCommentProxy.js` | ~~非表示 WebView プロキシ~~（iOS で動作せず無効化） |
